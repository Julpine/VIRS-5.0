...
public class OptimizedTextProcessorService implements TextProcessorService
{
	...
	public Text process(String textString)
	{
		...
		List<Word> resultWords = null;
		synchronized(this.lockRepo)
		{
			resultWords = wordRepository.findWordDistinctByValueIn(cleanValues);
		}//synchronized

		for(int i = 0; i < resultWords.size(); ++i)
		{
			Word w = resultWords.get(i);
			w.setValue(w.getValue().toLowerCase(Locale.ENGLISH));
		}//for i
